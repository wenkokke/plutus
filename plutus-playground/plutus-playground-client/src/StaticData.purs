...
import Types (Balance, MockWallet, Transfer)
...
{-# OPTIONS
  -fplugin=Language.Plutus.CoreToPLC.Plugin -fplugin-opt Language.Plutus.CoreToPLC.Plugin:dont-typecheck #-}
...
vestFunds vst value = do
  _ <-
    if value < totalAmount vst
      then otherError "Value must not be smaller than vested amount"
      else pure ()
...
validatorScriptHash =
  ...
  UTXO.getAddress . UTXO.scriptAddress . validatorScript

validatorScript :: Vesting -> Validator
validatorScript v = Validator val
  where
    val = UTXO.applyScript inner (UTXO.lifted v)
    inner =
      UTXO.fromPlcCode
        $(plutus
            [|\Vesting {..} () VestingData {..} (p :: PendingTx ValidatorHash) ->
                let eqBs :: ValidatorHash -> ValidatorHash -> Bool
                    eqBs = $(TH.eqValidator)
            ...
            -- Value that has been released so far under the scheme
                ...
                -- Nothing has been released yet
                    ...
                    paidOut =
                      let Value v = vestingDataPaidOut
                       in v
