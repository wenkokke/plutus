...
import           Wallet.API                 (WalletAPI (..), WalletAPIError, pubKey, signAndSubmit)
...
adjustMargin refs ft fd vl = do
    ...
    let
        ...
        fd' = if pk == futureDataLong fd
              then fd { futureDataMarginLong  = fromIntegral vl + futureDataMarginLong fd  }
              else fd { futureDataMarginShort = fromIntegral vl + futureDataMarginShort fd }
