...
updatePossibleActions oldState =
  let contract = oldState ^. (_contract <<< to (fromMaybe Refund))
      ...
      actions = extractRequiredActions contract
      actionInputs = map (actionToActionInput state) actions
  ...
  actionPerson (DepositInput _ accountId _ _) = (unwrap accountId).accountOwner
  actionPerson (ChoiceInput _ choiceId _ _) = (unwrap choiceId).choiceOwner
  -- We have a special person for notifications
  actionPerson (NotifyInput _ _) = "Notifications"
