...
newtype TextualProgram a = TextualProgram
    { unTextualProgram :: Program TyName Name a
    } deriving (Show)
...
propParser =
    property $ do
        ...
        let reprint =
                BSL.fromStrict .
                encodeUtf8 . prettyPlcDefText . unTextualProgram
...
propDeBruijn gen =
    property . hoist (return . runQuote) $ do
        (TermOf body _) <- forAllNoShowT gen
        let forward = deBruijnTerm
            backward ::
                   Except FreeVariableError (Term TyDeBruijn DeBruijn a)
                -> Except FreeVariableError (Term TyName Name a)
...
allTests testDir plcFiles rwFiles typeFiles typeNormalizeFiles typeErrorFiles =
    testGroup
        ...
        , testGroup
              "de Bruijn transformation round-trip"
...
testsNormalizeType =
    testGroup "golden type synthesis + normalization tests" .
...
testsGolden =
    ...
    fmap
        (asGolden (format $ defPrettyConfigPlcClassic defPrettyConfigPlcOptions))
...
testsRewrite =
    ...
    fmap
        (asGolden
             (format $ debugPrettyConfigPlcClassic defPrettyConfigPlcOptions))
...
testRebindCapturedVariable =
    let wName = TyName (Name () "w" (Unique 0))
        ...
        typeL2 =
            ...
            TyFun
                ()
                (TyForall () wName typeKind $
                 TyForall () xName typeKind (TyFun () varW varX))
                varZ
...
tests =
    ...
    fold
        ...
        , fmt "{- program " @?=
          Left
              (ParseErrorE
                   (LexErr "Error in nested comment at line 1, column 12"))
